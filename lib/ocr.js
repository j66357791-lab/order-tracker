...
// ---------------- 引擎调用（子进程，崩了不影响主站） ----------------
const enabled = () => process.env.OCR_ENABLED === '1';
function memOk() {
  try {
    const rss = process.memoryUsage().rss / 1048576;
    if (rss > 320) { ocrState = { available: false, reason: `主进程内存偏高（${Math.round(rss)}MB），本单转人工审核` }; return false; }
  } catch (e) {}
  return true;
}
function recognizeInChild(buf, timeoutMs) {
  return new Promise((resolve) => {
    ...
    let child;
    try {
      child = fork(CHILD, [], { execArgv: ['--max-old-space-size=160'], stdio: ['pipe', 'pipe', 'ignore', 'ipc'] });
    } catch (e) {
...
async function recognize(buf, timeoutMs = 25000) {
  if (!enabled()) {
    ocrState = { available: false, reason: 'OCR 未启用（默认关闭：设环境变量 OCR_ENABLED=1 可开启；当前充值全部转人工审核）' };
