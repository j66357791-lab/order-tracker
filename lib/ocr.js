...
function parseAmounts(text) {
  ...
  // 【2026-09-24 安全修复】带负号的是支出流水（账单页 -1.00），不是充值金额——
  // 原写法提取时丢掉负号，支出被当成正向金额候选，可能自动通过一笔实际是扣款的截图。负向候选一律排除。
  if (!out.length) {
    const re = /(?<![0-9.:])(-)?([0-9]{1,6}\.[0-9]{2})(?![0-9])/g;
    let m;
    while ((m = re.exec(t)) !== null) {
      if (m[1]) continue;   // 带负号：支出流水，跳过
      const v = Number(m[2]);
      if (Number.isFinite(v) && v > 0 && v < 1e7) out.push({ amount: v, raw: m[2], pri: 5 });
    }
  }
  const seen = new Set();
